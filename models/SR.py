...
class DSE(nn.Module):
    def __init__(self, num_filters=32):
        ...
        for _ in range(self.num_blocks):
            self.blocks.append(nn.Conv2d(num_filters, num_filters, 3, stride=1,padding=1))
...
class LightWeightBlock(nn.Module):
    def __init__(self, in_out_dim=40,dim=192,n_heads=8):
        super(LightWeightBlock, self).__init__()
        self.shift_sizes=[2 ,4, 2]
        self.blocks = nn.ModuleList()
        for size in self.shift_sizes:
            self.blocks.append(nn.Sequential(
                nn.Conv2d(in_out_dim, dim, 1, 1, 0),
                nn.GELU(),
                nn.Conv2d(dim, dim, 5, 1, 2, groups=dim),
                nn.GELU(),
                WinBasedAttention(dim=dim, num_heads=n_heads, window_size=8, shift_size=size),
                nn.Conv2d(dim, in_out_dim, 1, 1, 0)
            ))
...
class SRModel(nn.Module):
    def __init__(self, n_feats=40, n_heads=8, upscaling_factor=2):
        super(SRModel, self).__init__()
        self.scale = upscaling_factor 
        self.head = nn.Conv2d(3, n_feats, 3, 1, 1)
        
        self.body = LightWeightBlock(in_out_dim=n_feats, dim=40,n_heads=8)
